defmodule LoggerTest do
  ...
  test "Logger Call" do
    ...
    outputstr =
      capture_log(fn ->
        Maxwell.Middleware.Logger.call(
          conn,
          fn x -> {:error, "bad request", %{x | status: 400}} end,
          default: :info
        )
      end)
    ...
    output301 =
      capture_log(fn ->
        Maxwell.Middleware.Logger.call(conn, fn x -> %{x | status: 301} end, default: :error)
      end)

    output404 =
      capture_log(fn ->
        Maxwell.Middleware.Logger.call(conn, fn x -> %{x | status: 404} end, [{404, :debug}])
      end)

    output500 =
      capture_log(fn ->
        Maxwell.Middleware.Logger.call(conn, fn x -> %{x | status: 500} end, [
          {500..599, :warn},
          {:default, :error}
        ])
      end)
    ...
    outputok =
      capture_log(fn -> Maxwell.Middleware.Logger.call(conn, fn x -> x end, default: :info) end)

    nooutput =
      capture_log(fn ->
        Maxwell.Middleware.Logger.call(conn, fn x -> x end, [{400..599, :info}])
      end)
  ...
  test "Middleware.Logger with wrong options" do
    assert_raise ArgumentError,
                 "Logger Middleware Options doesn't accept wrong_option (:log_level)",
                 fn ->
                   defmodule TAtom2 do
                     use Maxwell.Builder, [:get, :post]
  ...
  test "Complex log_level with wrong status code" do
    assert_raise ArgumentError,
                 "Logger Middleware: status code only accepts Integer and Range.",
                 fn ->
