defmodule JsonTest do
  ...
  defmodule Client do
    ...
    middleware Maxwell.Middleware.Json,
      encode_func: &Poison.encode/1,
      decode_func: &Poison.decode/1,
      decode_content_types: ["text/html"],
  ...
  test "decode only if Content-Type is application/json" do
    assert "/invalid-content-type" |> Conn.new() |> Client.get!() |> Conn.get_resp_body() ==
             "hello"
  end
  ...
  test "do not encode tuple body" do
    assert Conn.put_req_body(Conn.new(), {:key, :vaule}) |> Client.post!() |> Conn.get_status() ==
             200
  end
...
defmodule DecodeJsonTest do
  ...
  test "EncodeJsonTest with wrong option" do
    assert_raise ArgumentError,
                 "EncodeJson Options doesn't accept wrong_option (:encode_func and :encode_content_type)",
                 fn ->
                   defmodule TAtom8 do
                     use Maxwell.Builder, [:get, :post]
  ...
  test "DecodeJsonTest with wrong option" do
    assert_raise ArgumentError,
                 "DecodeJson Options doesn't accept wrong_option (:decode_func and :decode_content_types)",
                 fn ->
