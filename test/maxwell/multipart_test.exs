defmodule Maxwell.MultipartTest do
  ...
  test "File Disposition" do
    ...
    {body, size} =
      Multipart.encode_form(boundary, [{:file, file_path, disposition, extra_headers}])
  ...
  test "File Content ExtraHeaders" do
    ...
    {body, size} =
      Multipart.encode_form(boundary, [{:file_content, file_content, filename, extra_headers}])
  ...
  test "File Content Disposition" do
    ...
    {body, size} =
      Multipart.encode_form(boundary, [
        {:file_content, file_content, filename, disposition, extra_headers}
      ])
  ...
  test "file_content content filename extra_headers stream len" do
    ...
    size =
      Multipart.len_mp_stream(boundary, [{:file_content, file_content, filename, extra_headers}])
  ...
  test "file_content content filename disposition extra_headers stream len" do
    ...
    size =
      Multipart.len_mp_stream(boundary, [
        {:file_content, file_content, filename, disposition, extra_headers}
      ])
  ...
  test "add_file_with_name/3 should add a file with name" do
    assert Multipart.new() |> Multipart.add_file_with_name("test.png", "media") ==
             {:multipart,
              [
                {:file, "test.png", {"form-data", [{"name", "media"}, {"filename", "test.png"}]},
                 []}
              ]}
  end
  ...
  test "add_file_content_with_name/4 should add a file_content with name" do
    assert Multipart.new() |> Multipart.add_file_content_with_name("xxx", "test.png", "media") ==
             {:multipart,
              [
                {:file_content, "xxx", "test.png",
                 {"form-data", [{"name", "media"}, {"filename", "test.png"}]}, []}
              ]}
  end
  ...
  test "add_file_content_with_name/5 should add a file_content with name and headers" do
    ...
    assert Multipart.new()
           |> Multipart.add_file_content_with_name("xxx", "test.png", "media", headers) ==
             {:multipart,
              [
                {:file_content, "xxx", "test.png",
                 {"form-data", [{"name", "media"}, {"filename", "test.png"}]}, headers}
              ]}
  end
